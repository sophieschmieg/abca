...
public abstract class AbstractRing<T extends Element<T>> implements Ring<T> {
	...
	public AbstractRing(boolean generateUnivariatePolynomialRing) {
		if (generateUnivariatePolynomialRing) {
			this.polynomialRing = new UnivariatePolynomialRing<T>(this);
		}
	}
	...
	public T divide(T dividend, T divisor) {
		return quotientAndRemainder(dividend, divisor).get(0);
	}

	public T remainder(T dividend, T divisor) {
		return quotientAndRemainder(dividend, divisor).get(1);
	}
	...
	public T lcm(T t1, T t2) {
		return quotientAndRemainder(multiply(t1, t2), gcd(t1, t2)).get(0);
	}
	...
	public List<T> extendedEuclidean(List<T> ts) {
		...
		List<T> end = extendedEuclidean(ts.subList(1, ts.size()));
		T gcd = end.get(0);
		List<T> start = extendedEuclidean(ts.get(0), gcd);
		result.add(start.get(0));
		result.add(start.get(1));
		for (int i = 1; i < end.size(); i++) {
			result.add(multiply(start.get(2), end.get(i)));
		}
		return result;
	}
