	...
	public TestFindRelatedCurves(Field<T> field, T a, T b, ProjectivePoint<T> generator) {
		...
		for (T bVariant : field) {
			...
			for (IntE prime : decomp.primeFactors()) {
				...
				int power = 1;
				while (power <= decomp.multiplicity(prime) && primePower.multiply(prime.getValue()).compareTo(max) <= 0) {
					power++;
					primePower = primePower.multiply(prime.getValue());
				}
				if (rcas.containsKey(prime.getValue()) && power <= rcas.get(prime.getValue()).power) {
					continue;
				}
				...
				for (ProjectivePoint<T> point : torsionPoints) {
					for (int pow = maxPower; pow <= power; pow++) {
						if (curve.multiply(prime.getValue().pow(pow), point).equals(curve.neutral())) {
							if (pow > maxPower) {
								maxPower = pow;
								torsionPoint = point;
							}
				...
				if (!rcas.containsKey(prime.getValue()) || rcas.get(prime.getValue()).power < maxPower) {
					if (rcas.containsKey(prime.getValue())) {
						RelatedCurveAttack<T> rca = rcas.get(prime.getValue());
						target.divide(rca.prime.pow(rca.power));
					}
					rcas.put(prime.getValue(), new RelatedCurveAttack<>(bVariant, prime.getValue(), maxPower, torsionPoint, curve));
					target = target.multiply(prime.getValue().pow(maxPower));
