...
public class ModuloNumberFieldIdeal extends AbstractRing<ModNFE> implements Ring<ModNFE> {
	...
	private List<ModuloNumberFieldIdeal> primePowerDecomposition;
	private List<NFE> chineseRemainderMultipliers;

	ModuloNumberFieldIdeal(NumberFieldIntegers order, NumberFieldIdeal ideal) {
...
//	}
	...
	public boolean isUnit(ModNFE t) {
		...
		for (ModuloNumberFieldIdeal mod : primePowerDecomposition()) {
	...
	public ModNFE inverse(ModNFE t) {
		...
		for (int i = 0; i < primePowerDecomposition().size(); i++) {
			ModuloNumberFieldIdeal mod = primePowerDecomposition().get(i);
	...
	public List<ModNFE> getUnitGenerators() {
		...
		if (unitGenerators == null) {
			...
			for (int i = 0; i < primePowerDecomposition().size(); i++) {
				...
				for (Vector<IntE> syzygy : primePowerDecomposition().get(i).genericModule.getSyzygies()) {
					List<IntE> augmented = new ArrayList<>();
					for (int j = 0; j < numGenerators; j++) {
						augmented.add(z.zero());
					}
					augmented.addAll(syzygy.asList());
