...
public class TestAsymmetricCrypto {
	...
	private PrimeFieldElement p256A;
	private PrimeFieldElement p256B;
	private EllipticCurve<PrimeFieldElement> p256;
	private ProjectivePoint<PrimeFieldElement> p256Generator;
	private BigInteger p256Order;
	private EllipticCurve<PrimeFieldElement> supersingularCurve;
	private Map<Integer, ProjectivePoint<PrimeFieldElement>> supersingularTorsionPoint;
	...
	public TestAsymmetricCrypto() {
		...
		fp256 = new PrimeField(p256Prime);
		p256A = fp256.getElement(new BigInteger("-3"));
		p256B = fp256.getElement(
				new BigInteger("41058363725152142129326129780047268409114441015993725554835256314039467401291"));
		PrimeFieldElement baseX = fp256.getElement(
				new BigInteger("48439561293906451759052585252797914202762949526041747995844080717082404635286"));
		PrimeFieldElement baseY = fp256.getElement(
	...
	public void doKeyAgreement() {
		...
		ProjectivePoint<PrimeFieldElement> publicKeyAlice = p256.multiply(privateKeyAlice, p256Generator);
	...
	public void doRelatedSupersingularCurveAttack(int numbits) {
		...
		KeyPair keyBob = makeECKeysJava();
		BigInteger privateKeyBob = ((ECPrivateKey) keyBob.getPrivate()).getS();
		ProjectivePoint<PrimeFieldElement> torsionPoint = null;

		if (supersingularTorsionPoint.containsKey(order)) {
			torsionPoint = supersingularTorsionPoint.get(order);
		} else {
			List<ProjectivePoint<PrimeFieldElement>> torsionPoints = supersingularCurve.getTorsionPoints(order);
			for (ProjectivePoint<PrimeFieldElement> p : torsionPoints) {
		...
		Map<ProjectivePoint<PrimeFieldElement>, Integer> results = new TreeMap<>();
		for (int i = 0; i < order; i++) {
			ProjectivePoint<PrimeFieldElement> mult = supersingularCurve.multiply(i, torsionPoint);
			results.put(mult, i);
		}
		ProjectivePoint<PrimeFieldElement> sharedPoint = p256.multiply(privateKeyBob, torsionPoint);
	...
	public ProjectivePoint<PrimeFieldElement> fromECPoint(ECPoint point) {
		if (point.equals(ECPoint.POINT_INFINITY)) {
			return p256.neutral();
		}
		return new ProjectivePoint<PrimeFieldElement>(fp256, fp256.getElement(point.getAffineX()),
				fp256.getElement(point.getAffineY()), fp256.one());
	}

	public ECPoint toECPoint(ProjectivePoint<PrimeFieldElement> point) {
	...
	public byte[] computeSharedSecret(BigInteger privateKey, ProjectivePoint<PrimeFieldElement> publicKey) {
		ProjectivePoint<PrimeFieldElement> sharedSecretPoint = p256.multiply(privateKey, publicKey);
