...
class Sha3Test {
	...
	void testEmpty() {
		...
		HashFunction[] hash = new HashFunction[] { Sha3.SHA3_224, Sha3.SHA3_256, Sha3.SHA3_384, Sha3.SHA3_512 };
		ExtendedOutputFunction[] xof = new ExtendedOutputFunction[] { Sha3.SHAKE_128, Sha3.SHAKE_256 };
		for (int i = 0; i < hash.length; i++) {
			assertEquals(hashResults[i], new ByteArray(hash[i].evaluate(new byte[0])).toString().toUpperCase());
		}
		for (int i = 0; i < xof.length; i++) {
			assertEquals(xofResults[i], new ByteArray(xof[i].evaluate(new byte[0], (xofResults[i].length() + 1) / 3))
	...
	void test1600() {
		...
		String[] hashResults = new String[] {
				...
				"18 81 DE 2C A7 E4 1E F9 5D C4 73 2B 8F 5F 00 2B " + "18 9C C1 E4 2B 74 16 8E D1 73 26 49 CE 1D BC DD "
						+ "76 19 7A 31 FD 55 EE 98 9F 2D 70 50 DD 47 3E 8F",
				"E7 6D FA D2 20 84 A8 B1 46 7F CF 2F FA 58 36 1B " + "EC 76 28 ED F5 F3 FD C0 E4 80 5D C4 8C AE EC A8 "
						+ "1B 7C 13 C3 0A DF 52 A3 65 95 84 73 9A 2D F4 6B "
						+ "E5 89 C5 1C A1 A4 A8 41 6D F6 54 5A 1C E8 BA 00" };
		String[] xofResults = new String[] {
				"13 1A B8 D2 B5 94 94 6B 9C 81 33 3F 9B B6 E0 CE " + "75 C3 B9 31 04 FA 34 69 D3 91 74 57 38 5D A0 37 "
				...
				"CD 8A 92 0E D1 41 AA 04 07 A2 2D 59 28 86 52 E9 " + "D9 F1 A7 EE 0C 1E 7C 1C A6 99 42 4D A8 4A 90 4D "
		...
		ExtendedOutputFunction[] xof = new ExtendedOutputFunction[] { Sha3.SHAKE_128, Sha3.SHAKE_256 };
		for (int i = 0; i < hash.length; i++) {
			assertEquals(hashResults[i], new ByteArray(hash[i].evaluate(input)).toString().toUpperCase());
		}
		for (int i = 0; i < xof.length; i++) {
			assertEquals(xofResults[i], new ByteArray(xof[i].evaluate(input, (xofResults[i].length() + 1) / 3))
					.toString().toUpperCase());
		}
	}
}
